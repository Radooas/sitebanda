  ...
  head: () => ({
    meta: [
      { title: "Bandas disponíveis para contratação — Zero82 Produções" },
      {
        name: "description",
        content:
          "Conheça nosso elenco de bandas, duos e DJs. Rock, jazz, acústico e eletrônica — escolha o som perfeito para seu evento.",
      },
      ...
      <main className="flex-1">
        <section className="container mx-auto px-6 py-20 md:py-28 text-center relative">
          ...
          <div className="relative">
            ...
            <p className="mt-6 text-lg text-muted-foreground max-w-2xl mx-auto">
              Selecionamos artistas com alta performance ao vivo, repertório versátil e postura
        ...
        <section className="container mx-auto px-6 pb-20 space-y-10">
          {bands.map((band, i) => (
            <article
              key={band.slug}
              className={`grid md:grid-cols-2 gap-8 items-center bg-card border border-border rounded-3xl overflow-hidden ${i % 2 ? "md:[&>div:first-child]:order-2" : ""}`}
            >
              ...
              <div className="p-8 md:p-12">
                ...
                <div className="mt-6 flex flex-wrap gap-4 text-sm">
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <Users className="h-4 w-4 text-primary" /> {band.members}{" "}
                    {band.members > 1 ? "integrantes" : "integrante"}
                  </div>
                  <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  {band.repertoire.map((r) => (
                    <span
                      key={r}
                      className="px-3 py-1 rounded-full text-xs border border-border bg-background/50"
                    >
                      {r}
                    </span>
                  ))}
                ...
                <Link
                  to="/contato"
                  className="mt-7 inline-flex items-center gap-2 rounded-full bg-gradient-gold px-6 py-3 text-sm font-semibold text-primary-foreground shadow-[var(--shadow-gold)] hover:scale-105 transition-transform"
                >
